...
(deftest rotation
  (let [a (:beacons (first dif-facing-input))
        b (:beacons (second dif-facing-input))]
    (is (= b (mapv #(day/rotate % [3 2 2]) a))))

  (->> dif-facing-input
       (map :beacons)
       (day/all-pairs)
       (map (fn [[a b]] (is (day/some-matching-rotation a b))))
       (doall)))

(def pa [1 3 6])
(def pb [78 4 2])
(deftest translation
  (is (= (day/translate pa (day/find-translation pa pb)) pb)))
